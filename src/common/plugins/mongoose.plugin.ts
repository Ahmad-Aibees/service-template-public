...
mongoose.plugin((schema: any) => {
    ...
    schema.pre(['find', 'findOne', 'findById', 'countDocuments'], function () {
        ...
        if (_id && typeof _id === 'string') this.getQuery()._id = mongoose.Types.ObjectId(_id);
    });

    // Update updated propery of documents
    schema.pre('findOneAndUpdate', function () {
        const set: {[key: string]: any} = this.getUpdate().$set || {};
    ...
    schema.pre('save', function () {
        if (this.isNew && !this._id) this._id = mongoose.Types.ObjectId();
    });
